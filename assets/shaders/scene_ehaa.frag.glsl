...
vec3 fresnelSchlick(float HdotVorL, vec3 F0) {
	...
	*/
	return F0 + (1.0 - F0) * /*Hdot4*invHdotL*/ pow(1.0 - HdotVorL, 5.0);
}
...
vec3 cookTorranceSpecular(float NdotV, float NdotL, float NdotH, float HdotVorL, vec3 albedo, float roughness, float metalness, out vec3 Ks) {
	float alpha = roughness*roughness;
	
	// Schlick F0
	const float ior = 1.47; // 1.47 is pretty representative for all kinds of materials
	float f = abs((1.0 - ior) / (1.0 + ior));
	vec3 F0 = mix(vec3(f*f), albedo, metalness); // Lerp to albedo color for conductors
	...
	vec3 specular = (D * F * G) / (4.0 * NdotV * NdotH/*NdotL*/ + EPSILON); // 4.0 apparently is the correct factor eventhough the original formula used pi
	Ks = F;
	return specular;
}
...
void main() {
	...
	/* per light */ {
		...
		vec3 Ks;
		vec3 specular = cookTorranceSpecular(NdotV, NdotL, NdotH, HdotL, albedo, roughness, metalness, Ks);
...
//	baryColor = vec2(1.0 - edgeCoverageFactor, 1.0);
	...
	*/
	...
	vec3 T, B;
	{
		...
		vec3 pos_dx = dFdx(tPositionWorldSpace.xyz);
		vec3 pos_dy = dFdy(tPositionWorldSpace.xyz);
		
		// Derivations of the texture coordinate
		vec2 texC_dx = dFdx(tModelTexCoord.st);
		vec2 texC_dy = dFdy(tModelTexCoord.st);
		
		// Tangent vector and binormal vector
		T = normalize(texC_dy.y * pos_dx - texC_dx.y * pos_dy);
		B = normalize(texC_dx.x * pos_dy - texC_dy.x * pos_dx);
	}
	vec3 N = normalize(tNormal);
...
//	vec3 B = normalize(tBitangent);
	...
	vec3 sampledNormal = 0.5 * texture(texMaterialNormal, tModelTexCoord.st).xyz + 0.5;
	N = normalize(normalMatrix * sampledNormal);
	*/
