...
//			let tex_normal = Texture::load_ktx_from_path(unsafe {&ASSET_MANAGER_INSTANCE}.resolve_asset_fs_path(&AssetPath::from_str("models/free_head/head_normal_bc1.ktx")).as_path(), ImageFormat::get(gl::COMPRESSED_RGB_S3TC_DXT1_EXT)).expect("Failed to load normal texture");
			let tex_normal = Texture::load_ktx_from_path(unsafe {&ASSET_MANAGER_INSTANCE}.resolve_asset_fs_path(&AssetPath::from_str("models/free_head/head_normal_bc7.ktx")).as_path(), ImageFormat::get(gl::COMPRESSED_SRGB_ALPHA_BPTC_UNORM_ARB)).expect("Failed to load normal texture");
